...
def main():
    print("=========================================")
    print("🚀 SNN Multi-Seed Robustness Validation (10 Seeds)")
    print("=========================================")
    ...
    print(f"{'Seed':^8} | {'p0(7)':^8} | {'p1(6)':^8} | {'p2(5)':^8} | {'p3(4)':^8} | {'Joint Acc':^10} | {'Hidden Nodes':^12} | {'Time(s)':^8}")
    print("-" * 88)
    ...
    for seed in seeds:
        ...
        # 轉換成百分比字串以美觀顯示
        print(f"{seed:^8d} | {bit_accs[0]*100:6.1f}% | {bit_accs[1]*100:6.1f}% | {bit_accs[2]*100:6.1f}% | {bit_accs[3]*100:6.1f}% | {joint_acc*100:8.1f}% | {hidden_count:^12d} | {t_elapsed:6.1f}s")
    ...
    # 儲存結果 JSON
    output_path = "multi_seed_results.json"
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=4)
    print(f"Successfully exported multi-seed results to: {output_path}")
